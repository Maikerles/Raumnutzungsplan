...
public class UnitController {
	...
	public String addPost(@Valid Unit unit, BindingResult result, Model model) {
	   ...
	   if (!unit.getName().matches("[0-9].[0-9]+")) {
		   result.rejectValue("name","error.namedoesnotmatchconvention" ,"Die angegebene Raumnummer enspricht nicht der Konvention");
		   return "unit/add";
	   }
	   ...
	   if (registeredUnit != null) {
	      return "redirect:/unit/list";
	       ...
	       log.error("Raumnummer vergeben: " + unit.getName());
	...
	public String edit(@PathVariable("id") Long id, Unit unit, Model model) {
		...
		unit.setBemerkung(u.getBemerkung());
	   return "unit/edit";
	}
	...
	public String editPost(@Valid Unit unit, BindingResult result, Model model) {
	   ...
	   if (result.hasFieldErrors("name") || result.hasFieldErrors("bemerkung")) {
	       return "unit/edit";
	   }
	   if (!unit.getName().matches("[0-9].[0-9]+")) {
		   result.rejectValue("name","error.namedoesnotmatchconvention" ,"Die angegebene Raumnummer enspricht nicht der Konvention");
