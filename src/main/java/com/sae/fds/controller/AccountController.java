...
public class AccountController {
	...
	@Autowired
	private InfoVersionService infoVersionSerivce;
	...
	@RequestMapping("/")
	public String home(Map<String, Object> model) throws FileNotFoundException {
		model.put("date", new Date());
		model.put("user", accountService.getLoggedInAccount());
		model.put("info", infoVersionSerivce.getCurrent());
	...
	public String registerPost(@Valid Account account, BindingResult result, ModelMap model) {
	...
	public String edit(@PathVariable("id") Long id, Account account, Model model) {
		...
		account.setProjectName(a.getProjectName());
		account.setProjectSupervisor(a.getProjectSupervisor());
	...
	public String editPost(@Valid Account account, BindingResult result) {
		...
		if (accountService.getLoggedInAccount().isAdmin()) {
			accountService.updateAccount(account.getUserName(), account);
		} else {
