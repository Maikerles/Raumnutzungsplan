...
public class EventService {
	...
	public Event add(Event event) {
		List<Event> events = eventRepository.findByDatesBetweenOr(event.getStart(), event.getEnd(), event.getTable().getId());
		if(events.size() == 0) {
	...
	public Event update(Event event) {
		List<Event> events = eventRepository.findByDatesBetweenOrAnd(event.getStart(), event.getEnd(), event.getTable().getId(), event.getId());
		if(events.size() == 0) {
